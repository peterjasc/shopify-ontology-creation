...
def process_pdfs_in_xlsx(input_filepath, output_filepath, row_to_start_from, title_column_header, collection_column_header):
 workbook = None
 try:
  ...
  collection_col_idx, found_collection_col, err_collection_col = get_excel_column_number(sheet, collection_column_header)
  if not found_collection_col:
   return f"{Colors.RED}Error: {err_collection_col}{Colors.RESET}"
  print(f"{Colors.GREEN}Value '{collection_column_header}' found in column number: {collection_col_idx}{Colors.RESET}")
  for row_index in range(row_to_start_from, sheet.max_row + 1):
   ...
   print(f"{Colors.CYAN}Row {row_index}: Title='{title_desc_value}' -> BERT Assigned Collection='{assigned_collection}'{Colors.RESET}")
...
def main():
 if not load_bert_model():
  print(f"{Colors.RED}BERT model not loaded. Please train the model using the 'Conceptual BERT Training Script' first, and ensure it's saved to '{MODEL_PATH}'.{Colors.RESET}")
  sys.exit(1)
 ...
 add_color(Colors.BLUE, f"Enter the row number to start processing from (1-based, press Enter for default: {default_row_to_start_from}): ")
 ...
 if row_input:
  try:
   ...
   if parsed_row < 1:
    print(f"{Colors.YELLOW}Row number must be 1 or greater. Using default: {default_row_to_start_from}{Colors.RESET}")
   else:
    row_to_start_from = parsed_row
  except ValueError:
   print(f"{Colors.YELLOW}Invalid input '{row_input}' for row number. Using default: {default_row_to_start_from}{Colors.RESET}")
